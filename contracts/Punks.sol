...
/**
  ...
  :vmo,=.  :,.+++=.:;  .;_qqs:  
  ;3mBms:;~=_,=++=_,;~:;%mWmo=, 
  :3WmWps%oSXc:|+`3SXosvmmBW1]v.
  :]#Wm#SoS22z:+|.n22SoX#mW#s]1`
  :)#mGSoS22Sn +=.S2SoSo2dBE}}~ 
   :3ZSoS22S2Ss_s<222S2S2S$e:   
   =]oS22S2222oXo2S2SoSoSo2(;   
   :vS22SoS2S2S2S222S22S2S2s:   
  .:SoS22S22222222S222SoSoSo:.  
  :)2S22SoS2S2S2S22S2S2S22S2;;  
 _:u22XmWpoSoSe}*322SommmXoSo:. 
.-<2S2d?WmX22S(=|)2S2XmW?S222>`:
.=2SoSo;?WSoSossvvS22dW(=S2S22;.
.=S222S( +22S2X22S22So^ )oS22S: 
 ;3oS2;\_>)So2"^""2So(<=%)o2Sn: 
 :)X2So,XSnSmh   .mq2oSo_oS22(: 
 :=SoS}2{e{mWBc .jmWmr1!2{22S`` 
  :3w+==/<m(WmW[]WB#]m/<;>)qr:. 
  -/$mQmmWW[]WW')#Wfj#QmmWmZ:`  
   -/$BWWWWmc~`. -~<WmWmWWZ':   
    -;4WBWBWB[:::;]WWWBWWF`:.   
     -_"9WWBW[:;=;]WBWmV^:`     
       -_/"9$[.==;]UT"',~       
...
contract PunkShiba is Context, IERC20, Ownable {
    ...
    address payable public liquidityAddress =
        payable(0x000000000000000000000000000000000000dEaD); // Liquidity incentive Address 
    ...
    uint256 private constant _tTotal = 999999999999999 * 1e18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "Punk Shiba";
    ...
    uint256 private minimumTokensBeforeSwap = _tTotal * 1 / 100000; // 0.001%
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "PUNKS: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "PUNKS: decreased allowance below zero"
            )
        );
        return true;
    }
    ...
    function _approve(
        ...
        require(owner != address(0), "PUNKS: approve from the zero address");
        require(spender != address(0), "PUNKS: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "PUNKS: transfer from the zero address");
        require(to != address(0), "PUNKS: transfer to the zero address");
        ...
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyBuybackFee <= 18, "Must keep taxes below 18%");
    }
        ...
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellBuybackFee <= 24, "Must keep taxes below 24%");
    }
